...
class SatSolver:
    ...
    def load_cnf_file(self, file_path):
    ...
    def verify_satisfiability(self):
    ...
    def resolve_clauses(self, cnf):
        ...
        while True:
            ...
            clauses = self.simplify_resolvents(clauses)
            if frozenset() in clauses:
                return False

    def simplify_resolvents(self, clauses):
    ...
    def _dpll_recursive(self):
        ...
        if not unassigned_vars:
            return self.verify_satisfiability()
...
def get_algorithm_choice():
    ...
    choices = input("\nEnter the numbers of algorithms you want to use (comma-separated, e.g., '1,2,3'): ")
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.cnf'):
            ...
            for algorithm in algorithms:
                solver = SatSolver()
                solver.load_cnf_file(filepath)
                start_time = time.time()
                try:
                    if algorithm == 'resolution':
                        result = solver.resolve_clauses(solver.clauses)
                    elif algorithm == 'dp':
                        result = solver.dp_algorithm()
                    elif algorithm == 'dpll':
                        result = solver.dpll_algorithm()
                    else:
                ...
                print(f"{algorithm.upper():<10} | {status:<6} | Time: {elapsed_time:.4f}s")
                results.append({
                    'file': filename,
                    'algorithm': algorithm,
                    'time': elapsed_time,
...
def print_benchmark_summary(results):
    ...
    print("{:<20} {:<10} {:<10} {:<10} {:<10} {:<10}".format(
        "File", "Algorithm", "Result", "Time(s)", "Variables", "Clauses"))
    print("-" * 80)
    for res in results:
        print("{:<20} {:<10} {:<10} {:<10.4f} {:<10} {:<10}".format(
    ...
    for algo in sorted(algorithms):
        ...
        if algo_times:
            ...
            print(f"{algo.upper():<10}: {avg_time:.4f}s (completed runs: {len(algo_times)})")
        else:
